...
def lookup_repo(repo_name: str):
    try:
        return github_service.find_installation_for_repo(repo_name)
...
def purge_sandboxes():
    ...
    try:
        return github_service.purge_all_sandboxes()
    except Exception as e:
...
async def clone_repository(req: github_schemas.CloneRequest, cleanup: bool = False):
    ...
    try:
        ...
        result = github_service.clone_repo(req.installation_id, req.owner, req.repo_name)
        ...
        if cleanup and local_path:
            github_service.cleanup_repo(local_path)

@router.post("/branch")
async def create_branch(req: github_schemas.BranchRequest):
    try:
        await resolve_gh_details(req)
        return github_service.create_branch(req.installation_id, req.owner, req.repo_name, req.base_branch, req.new_branch)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_file(req: github_schemas.FileRequest):
    try:
        await resolve_gh_details(req)
        result = github_service.find_file_in_repo(req.installation_id, req.owner, req.repo_name, req.file_path, req.ref)
        if "error" in result:
...
async def commit_file(req: github_schemas.CommitRequest):
    try:
        await resolve_gh_details(req)
        return github_service.commit_file_to_branch(
...
async def get_tree(req: github_schemas.FileRequest):
    try:
        await resolve_gh_details(req)
        return github_service.get_repo_tree(req.installation_id, req.owner, req.repo_name, req.ref)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def integrated_workflow(req: github_schemas.IntegratedWorkflowRequest):
    ...
    try:
        ...
        token = github_service.get_installation_access_token(req.installation_id)

        # 2. Clone Repo (Creates a unique sandbox)
        clone_result = github_service.clone_repo(req.installation_id, req.owner, req.repo_name, token=token)
